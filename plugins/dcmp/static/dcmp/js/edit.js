        ...
        if(field_type == "queue_pool"){
            ...
            for(var i=0; i<task_types.length; i++){
                ...
                if(task_type == "time_sensor"){
                    ...
                    field_html.push('<textarea ' + (readonly? ' readonly="readonly" ': '') + ' id="ace_' + task_id + '_' + task_type + '" class="form-control" rows="1" name="command">' + (task_type == task[field_name]? task["command"]: wait_task_instance_context_help) + '</textarea>');
                    field_html.push(render_help);
                    field_html.push(get_ace_script(task_type, "python"));
